    ...
    try:
        ...
        for retry in range(3):
            try:
                ...
                logger.error(f"Error fetching option parameters for {symbol} (attempt {retry+1}): {e}")
...
def add_fibonacci_targets(ib, symbol, spread_book, config):
    ...
    try:
        ...
        if df is None:
            logger.warning(f"Could not get historical data for {symbol} to calculate Fibonacci targets")
            return
