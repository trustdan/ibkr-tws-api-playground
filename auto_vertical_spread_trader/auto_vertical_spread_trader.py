...
def select_and_place(symbol, direction, bar, atr):
    ...
    try:
        ...
        for k1 in strikes:
            # pick OTM long leg
            if direction in ['bull', 'high_base'] and k1 < S: continue
            if direction in ['bear', 'low_base'] and k1 > S: continue
            ...
            if direction in ['bull', 'high_base'] and abs(delta) < CONFIG['MIN_DELTA']: continue
            if direction in ['bear', 'low_base'] and abs(delta) < CONFIG['MIN_DELTA']: continue
            if debit > CONFIG['MAX_COST']: continue
            if reward / debit < CONFIG['MIN_REWARD_RISK_RATIO']: continue
            ...
            try:
                ...
                logger.info(f"Placed {direction} spread for {symbol}: {k1}/{k2} {optType}, cost: ${debit:.2f}")
        ...
        try:
            for sym, info in list(spreadBook.items()):
                try:
                    ...
                    if info['type'] in ['bull', 'high_base'] and cur <= info['entryPrice'] - stop_diff:
                        logger.info(f"Stop triggered for {sym} {info['type']} at {cur}")
                        side = 'SELL'
                    elif info['type'] in ['bear', 'low_base'] and cur >= info['entryPrice'] + stop_diff:
                        logger.info(f"Stop triggered for {sym} {info['type']} at {cur}")
                        side = 'SELL'
                    else:
                        continue
                    ...
                    for opt in info['legs']:
                        try:
                            ...
                            logger.error(f"Error closing position for {sym} leg {opt.localSymbol}: {e}")
