...
def add_fibonacci_target(spread_info, df, extension_level=1.618):
    ...
    logger.info(
        f"Added Fibonacci target for {spread_info['symbol']}: "
...
def add_r_multiple_target(spread_info, r_multiple=2.0):
    ...
    stop_loss = (
        ...
        if direction in ["bull", "high_base"]
        else entry_price + (spread_info["ATR"] * spread_info["config"]["STOP_LOSS_ATR_MULT"])
    )
    ...
    logger.info(
        f"Added R-multiple target for {spread_info['symbol']}: "
...
def add_atr_target(spread_info, atr_multiple=3.0):
    ...
    logger.info(
        f"Added ATR-multiple target for {spread_info['symbol']}: "
