...
class AutoVerticalSpreadTrader:
    ...
    def __init__(
        self, config=CONFIG, use_cache=True, parallel=True, max_workers=4, paper_trading=False
    ):
        """
        ...
        for attempt in range(3):
            try:
                if not self.ib.isConnected():
                    ...
                    logger.info(
                        f"Connected to IB on {self.config['IB_HOST']}:{self.config['IB_PORT']}"
                    )
                return True
    ...
    def run_entries_if_time(self):
        ...
        if now.hour >= self.config["SCAN_HOUR_ET"] and self.last_run_date != now.date():
            try:
                ...
                self.filtered_universe = universe.filter_universe(
                    self.ib, self.universe, self.config
                )

                # Check if we're at position limit
                if len(self.spread_book) >= self.config["MAX_POSITIONS"]:
                    logger.warning(
                        f"Position limit reached ({len(self.spread_book)}/{self.config['MAX_POSITIONS']}). Skipping scan."
                    )
                    return False
                ...
                if space_available <= 0:
                    logger.info(
                        f"No space for new trades. Current positions: {len(self.spread_book)}"
                    )
                    return True
                ...
                for direction, sym, bar, atr in all_signals:
                    ...
                    # Place trade
                    if executor.select_and_place(
                        self.ib, sym, direction, bar, atr, self.config, self.spread_book
                    ):
                        trades_placed += 1
                ...
                logger.info(
                    f"Placed {trades_placed} new trades. Total positions: {len(self.spread_book)}"
                )
...
def parse_args():
    ...
    parser.add_argument(
        "--scan",
        choices=["bull_pullbacks", "bear_rallies", "high_base", "low_base", "all"],
        help="Run specific scan without trading",
    )
    parser.add_argument(
        "--symbols", help="Comma-separated list of symbols to scan instead of S&P 500"
    )
