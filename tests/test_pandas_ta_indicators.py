...
def test_momentum_indicators(price_data):
    ...
    assert (rsi.dropna() >= 0).all() and (rsi.dropna() <= 100).all(), "RSI values should be between 0 and 100"
...
def test_volatility_indicators(price_data):
    ...
    assert (bbands['BBU_20_2.0'].dropna() >= bbands['BBM_20_2.0'].dropna()).all(), "Upper band should be above middle band"
    assert (bbands['BBM_20_2.0'].dropna() >= bbands['BBL_20_2.0'].dropna()).all(), "Middle band should be above lower band"
