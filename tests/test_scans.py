...
class TestScanConditions(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.df['ATR14'] = self.df['high'].rolling(14).mean() - self.df['low'].rolling(14).mean()  # Simple proxy for ATR
        ...
        self.df['ATR_ratio'] = self.df['ATR14'] / self.df['ATR14'].rolling(20, min_periods=10).mean()
        self.df['range_pct'] = (self.df['high'] - self.df['low']) / self.df['close'] * 100
        self.df['range_ratio'] = self.df['range_pct'] / self.df['range_pct'].rolling(20, min_periods=10).mean()
