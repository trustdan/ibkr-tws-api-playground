...
class PerformanceMonitor:
        ...
        if not os.path.exists(self.trades_file):
            with open(self.trades_file, 'w', newline='') as file:
                ...
                writer.writerow([
                    ...
                    'stop_price', 'long_strike', 'short_strike', 'expiry',
                    'cost', 'profit', 'profit_pct', 'duration_days',
                    'exit_reason'
                ])
    ...
    def _init_scan_metrics_file(self):
        ...
        if not os.path.exists(self.scan_metrics_file):
            with open(self.scan_metrics_file, 'w', newline='') as file:
                writer = csv.writer(file)
                writer.writerow([
                    'date', 'scan_type', 'universe_size', 'signals_found',
    ...
    def _init_execution_metrics_file(self):
        ...
        if not os.path.exists(self.execution_metrics_file):
            with open(self.execution_metrics_file, 'w', newline='') as file:
                writer = csv.writer(file)
                writer.writerow([
                    'date', 'symbol', 'action', 'order_type',
                    'submission_time', 'execution_time', 'latency_ms',
    ...
    def record_trade(self, trade_data):
        ...
        logger.info(f"Recorded trade for {trade_data.get('symbol')}: "
                    f"P/L ${trade_data.get('profit', 0):.2f}")
    
    def record_scan_metrics(self, scan_type, universe_size, signals_found, execution_time_ms, memory_usage_mb):
        """
        ...
        logger.debug(f"Recorded scan metrics for {scan_type}: "
    ...
    def record_execution_metrics(self, execution_data):
        ...
        with open(self.execution_metrics_file, 'a', newline='') as file:
            ...
            writer.writerow([
                ...
                submission_time.isoformat() if isinstance(submission_time, datetime) else submission_time,
                execution_time.isoformat() if isinstance(execution_time, datetime) else execution_time,
                latency_ms,
                requested_price,
                executed_price,
                slippage
        ...
        logger.debug(f"Recorded execution metrics for {execution_data.get('symbol')}: "
    ...
    def generate_performance_report(self, lookback_days=30, save_plots=True):
        ...
        metrics = {
            ...
            'avg_win': recent_trades[recent_trades['profit'] > 0]['profit'].mean() if len(recent_trades[recent_trades['profit'] > 0]) > 0 else 0,
            'avg_loss': recent_trades[recent_trades['profit'] <= 0]['profit'].mean() if len(recent_trades[recent_trades['profit'] <= 0]) > 0 else 0,
            'max_win': recent_trades['profit'].max(),
            'max_loss': recent_trades['profit'].min(),
            'profit_factor': abs(recent_trades[recent_trades['profit'] > 0]['profit'].sum() / recent_trades[recent_trades['profit'] <= 0]['profit'].sum()) if recent_trades[recent_trades['profit'] <= 0]['profit'].sum() != 0 else float('inf'),
        ...
        for strategy in recent_trades['strategy'].unique():
            ...
            metrics['strategies'][strategy] = {
                ...
                'win_rate': len(strategy_trades[strategy_trades['profit'] > 0]) / len(strategy_trades) if len(strategy_trades) > 0 else 0,
        ...
        logger.info(f"Generated performance report: Win rate {metrics['win_rate']:.2%}, "
    ...
    def _generate_plots(self, trades_df):
        ...
        plt.figure(figsize=(10, 6))
        win_rates = trades_df.groupby('strategy').apply(
    ...
    def monitor_execution_latency(self, lookback_days=7):
        ...
        if alert_triggered:
            logger.warning(f"Execution latency alert: Recent average {recent_avg:.2f}ms "
